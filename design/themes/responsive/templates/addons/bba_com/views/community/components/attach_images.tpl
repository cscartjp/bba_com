...
<input type="hidden" name="{$name}_image_data{$suffix}[{$key}][pair_id]" value="{$pair.pair_id}"
       class="cm-image-field"/>
<input type="hidden" name="{$name}_image_data{$suffix}[{$key}][type]" value="{$type|default:"M"}"
       class="cm-image-field"/>
...
<div id="box_attach_images_{$name}_{$key}" class="attach-images">
    ...
    {hook name="attach_images:thumbnail"}
        <div class="upload-box clearfix {if $no_thumbnail && !$pair.icon}hidden{/if}"
             id="load_thumbnail_{$name}{$suffix}{$key}">
            {if $delete_pair && $pair.pair_id}
                <div class="float-right">
                    <a data-ca-target-id="box_attach_images_{$name}_{$key}"
                       href="{"image.delete_image_pair?pair_id=`$pair.pair_id`&object_type=`$object_type`"|fn_url}"
                       class="cm-confirm cm-post cm-ajax cm-tooltip pull-right" data-ca-event="ce.delete_image_pair"
                       title="{__("delete_image_pair")}"><i class="icon-remove"></i></a>
                </div>
            {/if}
            {if !$hide_titles}
                <h5>
            ...
            <div class="pull-left image-wrap">
                {if $pair.image_id}
                    {if !("MULTIVENDOR"|fn_allowed_for && $runtime.company_id && $object_type == "category")}
                        <a data-ca-target-id="image_{$pair.image_id}"
                           href="{"image.delete_image?pair_id=`$pair.pair_id`&image_id=`$pair.image_id`&object_type=`$object_type`"|fn_url}"
                           class="image-delete cm-confirm cm-post cm-ajax delete cm-delete-image-link cm-tooltip"
                           data-ca-event="ce.delete_image" title="{__("remove")}"><i class="icon-remove-sign"></i></a>
                    {/if}
                {/if}
                ...
                {if !$hide_images}
                    <div class="image">
                        {if $pair.image_id}
                            {include file="common/image.tpl" image_width="85" image_height="85" images=$pair.icon no_ids=true}
                        {else}
                ...
                {if !$hide_alt}
                    <div class="image-alt clear">
                        <div class="input-prepend">
                            <span class="add-on label-info cm-tooltip cm-hide-with-inputs" title="{__("alt_text")}"><i
                                        class="icon-comment"></i></span>
                            <input type="text" id="alt_icon_{$name}_{$key}"
                                   name="{$name}_image_data{$suffix}[{$key}][image_alt]" value="{$pair.icon.alt}"/>
                        </div>
                    </div>
                {/if}
            </div>
            ...
            <div class="upload-box clearfix">
                <div class="pull-left image-wrap">
                    {if !$hide_images}
                        {if $pair.detailed_id}
                            {if !("MULTIVENDOR"|fn_allowed_for && $runtime.company_id && $object_type == "category")}
                                <a data-ca-target-id="image_{$pair.detailed_id}"
                                   href="{"image.delete_image?pair_id=`$pair.pair_id`&image_id=`$pair.detailed_id`&object_type=detailed"|fn_url}"
                                   class="image-delete cm-confirm cm-post cm-tooltip cm-ajax delete cm-delete-image-link"
                                   data-ca-event="ce.delete_image" title="{__("remove")}"><i
                    ...
                    {if !$hide_alt}
                        <div class="image-alt">
                            <div class="input-prepend">
                                <span class="add-on label-info cm-tooltip cm-hide-with-inputs" title="{__("alt_text")}"><i
                                            class="icon-comment"></i></span>
                                <input type="text" id="alt_det_{$name}_{$key}"
                                       name="{$name}_image_data{$suffix}[{$key}][detailed_alt]"
                                       value="{$pair.detailed.alt}"/>
                            </div>
                        </div>
                    {/if}
                </div>
